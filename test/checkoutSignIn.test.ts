...
// Checkout wrapped its whole page in one <form>. The emailed-code sign-in
// card inside it has a form of its own, and a form inside a form reloaded the
// page when "Email me a sign-in code" was pressed: no code was sent, the
// email was wiped, and a shopper who was not signed in could never order.
...
/** Where each <EmailCodeSignIn sits, counted in open <form> elements around it. */
function signInFormDepths(source: string): number[] {
  const code = strip(source);
  const depths: number[] = [];
  let depth = 0;
  const tokens = /<form[\s>]|<\/form>|<EmailCodeSignIn\b/g;
