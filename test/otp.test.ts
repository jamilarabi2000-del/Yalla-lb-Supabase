...
describe('Supabase Email Authentication & Step-Up Security',()=>{
 it('uses password login plus Supabase email reauthentication OTP and no third-party dispatch',()=>{const guard=read('src/components/AdminGuard.tsx');expect(guard).toContain('supabase.auth.signInWithPassword');expect(guard).toContain('supabase.auth.reauthenticate');expect(guard).toContain("type: 'reauthentication'");expect(guard).toContain('verifyOtp');expect(guard).toContain('emailRedirectTo');expect(guard).toContain('cleanOtp.length !== 6');expect(guard).toContain('maxLength={6}');expect(guard).not.toContain('8-digit');expect(guard).not.toContain('PhoneMultiFactorGenerator');expect(guard).not.toContain('nodemailer');expect(guard).not.toContain('resend.com');expect(guard).not.toContain('recordAdminStepUp')});
