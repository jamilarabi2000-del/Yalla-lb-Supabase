...
describe('Supabase Admin Authentication',()=>{
 ...
 it('uses password authentication followed by Supabase email OTP verification',()=>{const guard=read('src/components/AdminGuard.tsx');expect(guard).toContain('signInWithPassword');expect(guard).toContain('adminOtpClient.auth.signInWithOtp');expect(guard).toContain("shouldCreateUser: false");expect(guard).toContain('verifyOtp');expect(guard).toContain("type: 'email'");expect(guard).toContain('cleanOtp.length < 6');expect(guard).toContain('cleanOtp.length > 10');expect(guard).toContain('maxLength={10}');expect(guard).not.toContain("type: 'reauthentication'")});
 ...
 it('gates the admin route on an authenticated admin session with a step-up',()=>{const guard=read('src/components/AdminGuard.tsx');const app=read('src/App.tsx');expect(guard).toContain("authStatus === 'authenticated_admin' && isMfaVerified");expect(guard).toContain('isMfaSessionValid');expect(guard).toContain('clearAdminMfaSession');expect(guard).toContain('signOutUser');expect(app).toContain('<AdminGuard><AdminView /></AdminGuard>');expect(app).not.toContain('AdminSessionGate')});
});
