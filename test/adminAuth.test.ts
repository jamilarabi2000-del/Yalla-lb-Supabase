...
describe('Supabase Admin Authentication',()=>{
 ...
 it('uses password authentication followed by Supabase reauthentication OTP',()=>{const guard=read('src/components/AdminGuard.tsx');expect(guard).toContain('signInWithPassword');expect(guard).toContain('supabase.auth.reauthenticate');expect(guard).toContain("type: 'reauthentication'");expect(guard).toContain('verifyOtp');expect(guard).toContain('emailRedirectTo');expect(guard).toContain('cleanOtp.length !== 6');expect(guard).toContain('maxLength={6}');expect(guard).not.toContain('8-digit')});
