...
describe('Supabase Admin Authentication',()=>{
 ...
 it('uses password authentication followed by Supabase email OTP verification',()=>{const guard=read('src/components/AdminGuard.tsx');expect(guard).toContain('signInWithPassword');expect(guard).toContain('signInWithOtp');expect(guard).toContain("shouldCreateUser: false");expect(guard).toContain('verifyOtp');expect(guard).toContain("type: 'email'");expect(guard).toContain('emailRedirectTo');expect(guard).toContain('cleanOtp.length < 6');expect(guard).toContain('cleanOtp.length > 10');expect(guard).toContain('maxLength={10}');expect(guard).not.toContain("type: 'reauthentication'")});
 ...
 it('blocks the admin route when an admin Supabase session has no valid step-up session',()=>{const gate=read('src/components/AdminSessionGate.tsx');const app=read('src/App.tsx');expect(gate).toContain("authStatus === 'authenticated_admin'");expect(gate).toContain('isMfaSessionValid');expect(gate).toContain('clearAdminMfaSession');expect(gate).toContain('signOutUser');expect(app).toContain('<AdminSessionGate>');expect(app).toContain('<AdminGuard><AdminView /></AdminGuard>')});
});
