-- Prevent incomplete catalog records from being made visible.
-- Drafts may be incomplete; publication is the completeness gate.
...
drop trigger if exists trg_validate_product_publish_requirements on public.products;
create trigger trg_validate_product_publish_requirements
before insert or update of is_published, publish_status, name, category_id, price_usd, stock, artisan, seller_item_code, image
on public.products
for each row execute function private.validate_publish_requirements();

drop trigger if exists trg_validate_category_publish_requirements on public.categories;
create trigger trg_validate_category_publish_requirements
before insert or update of is_published, name_en
on public.categories
for each row execute function private.validate_publish_requirements();

drop trigger if exists trg_validate_seller_publish_requirements on public.sellers;
create trigger trg_validate_seller_publish_requirements
before insert or update of is_active, name_en
on public.sellers
for each row execute function private.validate_publish_requirements();
