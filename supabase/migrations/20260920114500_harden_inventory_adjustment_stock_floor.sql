-- Prevent inventory adjustments from writing a negative ledger delta while silently clamping stock to zero.
-- Lock the product row so the ledger and stock mutation remain atomic under concurrency.
...
begin
  ...
  if p_quantity_change < 0 and v_stock + p_quantity_change < 0 then
  ...
  values (
    p_product_id,v_seller,p_quantity_change,p_reason,p_reference_type,p_reference_id,p_note,v_actor
  )
