        ...
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden space-y-3">
          ...
          >
            ...
            <div className="space-y-1 pt-2 border-t border-slate-100">
              ...
              <nav className="space-y-0.5">
                {[
                  { id: 'db_logs' as AdminMenuTab, label: 'Database Sync & Logs', icon: Zap, color: 'text-emerald-500', tag: 'Live' },
                  { id: 'inventory' as AdminMenuTab, label: 'Inventory Ledger', icon: Box, color: 'text-blue-500' },
                  { id: 'builder' as AdminMenuTab, label: 'Visual Builder', icon: PanelsTopLeft, color: 'text-violet-500' },
                  { id: 'analytics' as AdminMenuTab, label: 'Analytics', icon: BarChart3, color: 'text-indigo-500' },
                  { id: 'notifications' as AdminMenuTab, label: 'Notifications', icon: Bell, color: 'text-amber-500' },
                  { id: 'security' as AdminMenuTab, label: 'Security & Audits', icon: Database, color: 'text-rose-500' }
                ].map((item) => {
                  const isActive = currentTab === item.id;
                  ...
                  return (
                    <button
                      ...
                      className={`
                        w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-[13px] font-medium transition-all cursor-pointer group text-left
                        ${isActive ? 'bg-indigo-50/90 text-indigo-700 font-bold shadow-xs border-l-3 border-indigo-600' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}
                      `}
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      {item.tag && (
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider shrink-0 ${isActive ? 'bg-indigo-600 text-white' : 'bg-emerald-100 text-emerald-800'}`}>{item.tag}</span>
                      )}
                    </button>
                  );
                })}
