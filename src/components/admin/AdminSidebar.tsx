    ...
    return (
      ...
      >
        ...
        {item.badge !== undefined && (
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-black transition-colors shrink-0 ${isActive ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white'}`}>{item.badge}</span>
        )}
        {item.tag && (
          <span className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider shrink-0 ${isActive ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{item.tag}</span>
        )}
      </button>
    );
  };
      ...
      <aside className={`fixed top-0 bottom-0 left-0 z-50 w-72 h-screen max-h-screen bg-white border-r border-slate-200/80 flex flex-col justify-between py-4 px-3 transition-transform duration-200 ease-in-out lg:translate-x-0 lg:sticky lg:top-0 lg:h-screen lg:shrink-0 shadow-sm ${isOpenMobile ? 'translate-x-0 shadow-2xl ring-1 ring-slate-900/10' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden space-y-3">
          <div className="flex items-center gap-3 px-2 pt-1 pb-1 flex-shrink-0">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-indigo-700 to-indigo-500 flex items-center justify-center text-white font-black text-base tracking-wider shadow-md shadow-indigo-500/25 ring-2 ring-indigo-100">PA</div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5"><h1 className="text-base font-extrabold text-slate-900 tracking-tight leading-tight truncate">PlainAdmin</h1><span className="text-[10px] font-black uppercase px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-700 border border-indigo-100/80">PRO</span></div>
              <div className="flex items-center gap-1.5 mt-0.5"><span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span></span><span className="text-[11px] font-bold text-emerald-600 tracking-tight">Supabase Connected</span></div>
            </div>
          </div>
          ...
          <div ref={navScrollRef} className="flex-1 min-h-0 overflow-y-auto space-y-4.5 pr-1.5 -mr-1.5 overscroll-contain focus:outline-none scroll-smooth pb-4" style={{ scrollbarWidth: 'thin', scrollbarColor: '#cbd5e1 transparent' }}>
            ...
            <div className="space-y-1 pt-2 border-t border-slate-100">
              <div className="flex items-center justify-between px-3 pb-1"><span className="text-[10px] font-black tracking-widest text-slate-400 uppercase">Page Content & CMS</span><span className="text-[9px] font-black uppercase px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600 border border-indigo-100/60">Live</span></div>
              <nav className="space-y-0.5">{pageContentItems.map(item => renderNavButton(item, true))}</nav>
            </div>
            <div className="space-y-1 pt-2 border-t border-slate-100">
              <div className="flex items-center justify-between px-3 pb-1"><span className="text-[10px] font-black tracking-widest text-slate-400 uppercase">System & Diagnostics</span><span className="flex items-center gap-1 text-[9px] font-black uppercase px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-100/60"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>Active</span></div>
              <nav className="space-y-0.5">{renderNavButton({ id: 'db_logs', label: 'Database Sync & Logs', icon: '⚡', tag: 'Stream' })}</nav>
            </div>
          </div>
        </div>
        <div className="pt-3.5 border-t border-slate-100 space-y-2 flex-shrink-0">
          ...
          <button onClick={goBack} className="w-full flex items-center justify-center gap-2 px-3.5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold tracking-wide transition-all cursor-pointer shadow-xs"><ArrowLeft className="w-3.5 h-3.5" /><span>Storefront</span></button>
