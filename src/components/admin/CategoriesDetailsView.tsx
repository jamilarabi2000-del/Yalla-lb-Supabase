        ...
        {activeTab === 'categories' ? (
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="hidden lg:flex items-center gap-1 bg-slate-100 p-1 rounded-xl text-xs">
              <span className="text-[10px] font-bold text-slate-600 uppercase px-1.5 flex items-center gap-1">
            ...
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl text-xs">
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold transition-all cursor-pointer ${filterPublished === 'all' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600'}`}
              >
                All ({categories.length})
              </button>
              <button
                onClick={() => setFilterPublished('published')}
                className={`px-3 py-1 rounded-lg font-bold transition-all cursor-pointer ${filterPublished === 'published' ? 'bg-white text-emerald-700 shadow-xs' : 'text-slate-600'}`}
              >
                Published ({categories.filter(c => c.isPublished !== false).length})
              </button>
              <button
                onClick={() => setFilterPublished('hidden')}
                className={`px-3 py-1 rounded-lg font-bold transition-all cursor-pointer ${filterPublished === 'hidden' ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-600'}`}
              >
        ...
        <div className="bg-gradient-to-r from-indigo-900/90 via-slate-900 to-slate-900 text-white p-3.5 sm:p-4 rounded-2xl border border-indigo-500/20 shadow-sm space-y-2">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-thin">
            {categories.map((cat, idx) => (
              ...
              >
                <span className="w-4 h-4 rounded-full bg-indigo-500/50 text-[10px] font-mono font-bold flex items-center justify-center text-white">
                  {idx + 1}
                </span>
                <span>{cat.icon}</span>
                <span className="font-semibold text-white truncate max-w-[120px]">{cat.nameEn}</span>
                <span className="text-[10px] text-indigo-100 font-serif">({getProductCountForCategory(cat.id)})</span>
              </div>
            ))}
          </div>
        </div>
                            ...
                            <button
                              ...
                              className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-black transition-all cursor-pointer ${
                                isFirst 
                                  ...
                                  : 'bg-amber-500 hover:bg-amber-400 text-slate-900 shadow-xs'
                              }`}
                            >
                      ...
                      <div className="space-y-3">
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-[#7d6230] font-serif font-bold">{cat.nameAr}</p>
                                </div>
                              </div>
                        ...
                        <div className="space-y-1.5 pt-2 border-t border-slate-100">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-[#7d6230] flex items-center gap-1">
                ...
                <div key={reg.id} className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs space-y-4 flex flex-col justify-between hover:border-amber-200 transition-all">
                  <div className="space-y-3">
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
                      ...
                      <span className={`font-bold ${reg.expressAvailable ? 'text-emerald-700' : 'text-slate-500'}`}>
                      ...
                      return (
                        <button
                          ...
                          className={`px-2 py-0.5 rounded-md text-[11px] font-serif transition-all ${
                            alreadyAdded 
                              ? 'bg-amber-200/50 text-amber-700 cursor-not-allowed opacity-60' 
