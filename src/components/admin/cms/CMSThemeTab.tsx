              ...
              return (
                <details key={slot.id} className="rounded-2xl border border-slate-200 bg-white" open={slot.id === 'body'}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 p-4 border-t border-slate-100">
                    ...
                    <label className="text-xs text-slate-600">Alignment<SearchableSelect value={s.textAlign || 'start'} onChange={e=>updateTextStyle(slot.id,'textAlign',e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-xs"><option value="start">Start</option><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option><option value="end">End</option></SearchableSelect></label>
                    ...
                    <label className="text-xs text-slate-600">Wrapping<SearchableSelect value={s.whiteSpace || 'normal'} onChange={e=>updateTextStyle(slot.id,'whiteSpace',e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-xs"><option value="normal">Wrap</option><option value="nowrap">No wrap</option><option value="pre-wrap">Preserve + wrap</option></SearchableSelect></label>
                    <label className="text-xs text-slate-600">Overflow<SearchableSelect value={s.overflow || 'visible'} onChange={e=>updateTextStyle(slot.id,'overflow',e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-xs"><option value="visible">Visible</option><option value="hidden">Hidden</option></SearchableSelect></label>
                    <label className="text-xs text-slate-600">Text overflow<SearchableSelect value={s.textOverflow || 'clip'} onChange={e=>updateTextStyle(slot.id,'textOverflow',e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-xs"><option value="clip">Clip</option><option value="ellipsis">Ellipsis</option></SearchableSelect></label>
                    <label className="text-xs text-slate-600">Transform<SearchableSelect value={s.textTransform || 'none'} onChange={e=>updateTextStyle(slot.id,'textTransform',e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-xs"><option value="none">None</option><option value="uppercase">UPPERCASE</option><option value="lowercase">lowercase</option><option value="capitalize">Capitalize</option></SearchableSelect></label>
                  </div>
                </details>
              );
            })}
