        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {leaderboard.map((r, i) => <div key={r.query} className="p-4 flex items-center gap-3 hover:bg-slate-50/60">
              ...
              <button onClick={() => copyQuery(r.query)} className="p-2 rounded-xl text-slate-400 hover:bg-slate-100" title="Copy Query" aria-label={`Copy "${r.query}"`}>{copied === r.query ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}</button>
            </div>)}
        ...
        <aside className="space-y-5">
          <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
            ...
            <div className="max-h-[460px] overflow-y-auto divide-y divide-slate-100">
              {rows.slice(0,20).map(r => <div key={r.id} className="p-3.5"><div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-slate-900 truncate">{r.query}</span><span className="text-[10px] text-slate-400 whitespace-nowrap">{relativeTime(r.timestamp)}</span></div><div className="mt-1 flex items-center gap-1.5">{r.userName ? <span className="text-[9px] font-black uppercase tracking-wider text-slate-700">{r.userName}</span> : <span className="px-1.5 py-0.5 rounded bg-slate-100 text-[9px] font-black uppercase tracking-wider text-slate-500">Guest</span>}{r.origin && <span className="text-[9px] text-slate-400">• {r.origin}</span>}</div></div>)}
