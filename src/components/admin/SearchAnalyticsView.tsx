...
import {
  Search, RefreshCw, Activity, Flame, Languages, Clock3, Copy, Check,
  ...
  return (
    <section className="space-y-5">
      <header className="bg-white p-6 sm:p-7 rounded-3xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div><h2 className="text-lg sm:text-xl font-black text-slate-900">Search Trends & Discovery</h2><p className="text-xs font-semibold text-slate-500">Shopper intent and demand intelligence for Yalla Lebanon.</p></div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-emerald-50 text-emerald-700 border border-emerald-200"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />{live ? 'Live Sync' : 'Connecting'}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          [Search, 'Total Searches', filteredRows.length, 'bg-indigo-50 text-indigo-600'],
          [Users, 'Unique Keywords', leaderboard.length, 'bg-amber-50 text-[#b89753]'],
          [Flame, 'Top Search', top ? top.query : '—', 'bg-emerald-50 text-emerald-700'],
          [Clock3, '24-Hour Velocity', velocity24, 'bg-violet-50 text-violet-700']
        ].map(([Icon, label, value, tone]: any) => <div key={label} className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs"><div className={`w-9 h-9 rounded-xl flex items-center justify-center ${tone}`}><Icon className="w-4 h-4" /></div><p className="mt-3 text-[10px] font-black uppercase tracking-wider text-slate-500">{label}</p><p className="mt-1 text-2xl sm:text-3xl font-black text-slate-900 truncate">{value}</p>{label === 'Top Search' && top && <p className="text-[10px] font-bold text-slate-500 mt-1">{top.share.toFixed(1)}% of searches</p>}</div>)}
      </div>
      ...
      <div className="bg-slate-100/70 p-2 rounded-2xl border border-slate-200/80 flex flex-col lg:flex-row gap-2">
        ...
        <div className="relative flex-1">
          ...
          {filter && <button onClick={() => setFilter('')} className="absolute right-2 top-2 p-1 text-slate-400"><X className="w-3.5 h-3.5" /></button>}
        </div>
        ...
        <button onClick={() => setClearOpen(true)} className="px-4 py-2.5 rounded-xl bg-rose-50 text-rose-700 border border-rose-200 text-xs font-bold hover:bg-rose-100"><Trash2 className="w-3.5 h-3.5 inline mr-1" />Clear</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)] gap-5">
        <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {leaderboard.map((r, i) => <div key={r.query} className="p-4 flex items-center gap-3 hover:bg-slate-50/60">
              ...
              <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="text-sm font-bold text-slate-900 truncate">{r.query}</span>{isArabic(r.query) && <span className="px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-[9px] font-black">عربي</span>}</div><div className="mt-2 w-full bg-slate-100 rounded-full h-1.5 overflow-hidden"><div className="bg-amber-500 h-1.5 rounded-full transition-all" style={{width: `${r.share}%`}} /></div></div>
              <span className="px-2.5 py-1 rounded-full text-xs font-black bg-amber-50 text-[#b89753] border border-amber-200/80">{r.count}</span>
              <button onClick={() => inspect(r.query)} className="p-2 rounded-xl text-[#b89753] hover:bg-amber-50" title="Inspect in Store"><ExternalLink className="w-4 h-4" /></button>
              <button onClick={() => copyQuery(r.query)} className="p-2 rounded-xl text-slate-400 hover:bg-slate-100">{copied === r.query ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}</button>
            </div>)}
        ...
        <aside className="space-y-5">
          <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex items-center gap-2"><Activity className="w-4 h-4 text-emerald-600" /><h3 className="font-black text-slate-900">Live Search Stream</h3></div>
            <div className="max-h-[460px] overflow-y-auto divide-y divide-slate-100">
              {rows.slice(0,20).map(r => <div key={r.id} className="p-3.5"><div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-slate-900 truncate">{r.query}</span><span className="text-[10px] text-slate-400 whitespace-nowrap">{relativeTime(r.timestamp)}</span></div><div className="mt-1 flex items-center gap-1.5"><span className="text-[9px] font-black uppercase tracking-wider text-slate-500">{r.userName || 'Guest Shopper'}</span>{r.origin && <span className="text-[9px] text-slate-400">• {r.origin}</span>}</div></div>)}
