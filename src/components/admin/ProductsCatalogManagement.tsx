    ...
    return <article className="bg-white border border-slate-200 rounded-2xl p-3 shadow-sm hover:shadow-md transition-shadow">
      <div className={`rounded-xl border ${index === 0 && viewMode === 'sequence' ? 'border-amber-300 bg-amber-50/40' : 'border-slate-200'} p-2 mb-2`}>
        <div className="flex items-center gap-2 text-[11px] font-bold">
          ...
          <span className="px-2 py-0.5 rounded-full bg-slate-900 text-white">#{index + 1}</span>
          {index === 0 ? <button onClick={() => makeFirst(index)} className="ml-auto px-2 py-1 rounded-full bg-amber-400 text-white text-[10px] font-black"><Crown className="inline w-3 h-3 mr-1"/>#1</button> : <button onClick={() => makeFirst(index)} className="ml-auto px-2 py-1 rounded-full bg-amber-400/90 text-white text-[10px] font-black">Make #1</button>}
          <button onClick={() => move(index, -1)} className="p-1 text-slate-400 hover:text-indigo-600" title="Move up"><ArrowUp className="w-3.5 h-3.5"/></button>
          <button onClick={() => move(index, 1)} className="p-1 text-slate-400 hover:text-indigo-600" title="Move down"><ArrowDown className="w-3.5 h-3.5"/></button>
        </div>
      ...
      <div className="relative rounded-xl bg-slate-50 border border-slate-200 overflow-hidden aspect-[1.18/1] flex items-center justify-center">
        ...
        <span className="absolute top-2 right-2 px-2 py-1 rounded-full bg-slate-900 text-white text-[11px] font-black shadow">${Number(p.priceUSD || 0).toFixed(2)}</span>
      ...
      <div className="pt-3">
        ...
        <div className="flex items-center justify-between mt-2 text-[10px]">
          ...
          <span className="text-slate-400 truncate max-w-[120px]">{p.category}</span>
        </div>
  ...
  return <section className="space-y-4 text-slate-900">
    <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm">
      ...
      <div className="flex flex-wrap gap-2 mt-5">
        ...
        <button onClick={()=>bulkPublish(true)} disabled={!selected.size} className="px-3 py-2 rounded-xl bg-emerald-600 text-white text-[11px] font-black flex items-center gap-1.5 disabled:opacity-50"><CheckCircle2 className="w-3.5 h-3.5"/>Public Publish Live</button>
        <button onClick={()=>{setForm(emptyProduct());setEditing({} as Product);}} className="px-4 py-2 rounded-xl bg-indigo-600 text-white text-[11px] font-black flex items-center gap-1.5"><Plus className="w-3.5 h-3.5"/>ADD PRODUCT</button>
      </div>
    </div>

    <div className="bg-white border border-slate-200 rounded-2xl p-2.5 shadow-sm flex flex-col xl:flex-row gap-2.5">
      <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search by title, seller / artisan, origin, category, keywords..." className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-slate-200 text-xs outline-none focus:border-indigo-400"/></div>
    ...
    {!sequence.length && <div className="bg-white border border-dashed border-slate-300 rounded-2xl p-12 text-center"><AlertTriangle className="w-7 h-7 mx-auto text-slate-300"/><p className="mt-2 text-sm font-bold text-slate-500">No products match the current filters.</p></div>}
    {editing && <Modal/>}
  </section>;
};
