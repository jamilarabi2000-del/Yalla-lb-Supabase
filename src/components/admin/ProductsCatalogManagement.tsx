...
export const ProductsCatalogManagement: React.FC = () => {
  ...
  const RequiredBadge = ({ field }: { field: string }) => validationErrors[field] ? <span className="ml-2 px-1.5 py-0.5 rounded-full bg-rose-100 text-rose-700 text-[9px] font-black">REQUIRED</span> : <span className="ml-2 text-[9px] text-slate-500 font-bold">Required</span>;
    ...
    return <article className="bg-white border border-slate-200 rounded-2xl p-3 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="pt-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className={`shrink-0 px-2 py-0.5 rounded-full text-[9px] font-black ${published ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{published ? 'LIVE' : 'HIDDEN'}</span>
        </div>
        ...
        <div className="flex items-center justify-between mt-2 text-[10px]">
          <span className={`font-bold ${stockState === 'out' ? 'text-rose-600' : stockState === 'low' ? 'text-amber-700' : 'text-emerald-700'}`}>{stockState === 'out' ? 'Out of stock' : stockState === 'low' ? 'Low stock' : `${p.stock} in stock`}</span>
        ...
        <div className="flex gap-1.5 mt-3 pt-2 border-t border-slate-100">
          ...
          }} className="p-1.5 rounded-lg bg-emerald-50 text-emerald-700" title={published ? 'Hide' : 'Publish'}>{published ? <EyeOff className="w-3.5 h-3.5"/> : <Eye className="w-3.5 h-3.5"/>}</button>
      ...
      <div className="bg-white rounded-3xl w-full max-w-6xl max-h-[96vh] overflow-hidden shadow-2xl border border-slate-200 flex flex-col min-h-0">
        <div className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-slate-200 px-5 py-4 sm:px-7">
          <div className="flex items-start justify-between gap-4">
            ...
            <button onClick={close} className="p-2.5 rounded-xl hover:bg-slate-100 text-slate-600"><X className="w-5 h-5"/></button>
          </div>
        </div>
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain p-5 sm:p-7 space-y-7" style={{ WebkitOverflowScrolling: "touch" }}>
          <section><div className="flex items-center gap-2 mb-3"><span className="w-7 h-7 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center text-xs font-black">1</span><div><h4 className="font-black text-slate-900">Bilingual Product Identity</h4><p className="text-[11px] text-slate-500">The commercial identity used across the international and Arabic catalog.</p></div></div>
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <label className="text-xs font-black text-slate-600 md:col-span-2">Category Selection *<SearchableSelect value={form.category || ''} onChange={e=>{setField('category',e.target.value);if(e.target.value.trim())setValidationErrors(v=>({...v,category:''}));}} className={fieldClass('category','mt-1.5 w-full px-3 py-2.5 rounded-xl border bg-white')}>{!form.category && <option value="">Select a bilingual catalog category…</option>}{categories.map((cat:any)=><option key={cat.id} value={cat.id}>{cat.icon ? cat.icon+' ' : ''}{cat.nameEn} {cat.nameAr ? '— '+cat.nameAr : ''}</option>)}</SearchableSelect>{selectedCategory && <span className="block mt-1.5 text-[10px] text-indigo-600 font-bold">{selectedCategory.icon} {selectedCategory.nameEn} · {selectedCategory.nameAr}</span>}</label>
            </div>
          </section>
          ...
          <section><div className="flex items-center gap-2 mb-3"><span className="w-7 h-7 rounded-lg bg-amber-50 text-amber-700 flex items-center justify-center text-xs font-black">2</span><div><h4 className="font-black">Artisan &amp; Terroir Origin Linkage</h4><p className="text-[11px] text-slate-500">Select a registered Lebanese seller to automatically link producer identity and terroir.</p></div></div>
            <div className="grid md:grid-cols-2 gap-4">
              <label className="text-xs font-black text-slate-600 md:col-span-2">Registered Seller / Artisan<SearchableSelect value={form.sellerId || ''} onChange={e=>setSeller(e.target.value)} className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-white"><option value="">Independent / select later</option>{sellers.filter((s:any)=>s.isActive !== false).map((s:any)=><option key={s.id} value={s.id}>{s.nameEn}{s.nameAr ? ' — '+s.nameAr : ''}{s.region ? ' · '+s.region : ''}</option>)}</SearchableSelect>{selectedSeller && <div className="mt-2 flex flex-wrap gap-1.5"><span className="px-2 py-1 rounded-full bg-amber-50 text-amber-700 text-[10px] font-black">{selectedSeller.nameEn}</span><span dir="rtl" className="px-2 py-1 rounded-full bg-slate-100 text-slate-700 text-[10px]">{selectedSeller.nameAr || 'Arabic name not registered'}</span><span className="px-2 py-1 rounded-full bg-emerald-50 text-emerald-700 text-[10px]">Terroir: {selectedSeller.region}</span></div>}</label>
          ...
          <section><div className="flex items-center gap-2 mb-3"><span className="w-7 h-7 rounded-lg bg-emerald-50 text-emerald-700 flex items-center justify-center text-xs font-black">9</span><div><h4 className="font-black">Bilingual Search Engine Optimization (SEO)</h4><p className="text-[11px] text-slate-500">Keywords support Google metadata and storefront search indexing.</p></div></div>
            <div className="grid lg:grid-cols-2 gap-4"><div><label className="text-xs font-black text-slate-600">Arabic SEO Keywords (الكلمات الدلالية لمحركات البحث)</label><div className="mt-1.5 min-h-12 rounded-xl border border-slate-200 p-2 flex flex-wrap gap-1.5">{arabicKeywords.map(k=><span dir="rtl" key={k} className="px-2 py-1 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-bold">{k}<button type="button" onClick={()=>removeArabicKeyword(k)} className="ml-1 text-emerald-700">×</button></span>)}<input dir="rtl" onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();const v=e.currentTarget.value.trim();if(v)addArabicKeyword(v);e.currentTarget.value='';}}} placeholder="اكتب كلمة واضغط Enter" className="flex-1 min-w-36 outline-none text-xs text-right"/></div><div className="flex flex-wrap gap-1.5 mt-2">{arabicQuickKeywords.map(k=><button type="button" key={k} onClick={()=>addArabicKeyword(k)} className="px-2 py-1 rounded-full bg-slate-50 text-slate-600 text-[9px] font-bold">{k}</button>)}</div></div><label className="text-xs font-black text-slate-600">English SEO Keywords<textarea rows={3} value={form.keywordsInput || ''} onChange={e=>setField('keywordsInput',e.target.value)} placeholder="lebanese zaatar, mouneh, cold pressed olive oil, handmade" className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200"/></label></div>
  ...
  return <section className="space-y-4 text-slate-900">
    <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm">
      ...
      <div className="flex flex-wrap gap-2 mt-5">
        ...
        <button onClick={()=>bulkPublish(true)} disabled={!selected.size} className="px-3 py-2 rounded-xl bg-emerald-700 text-white text-[11px] font-black flex items-center gap-1.5 disabled:opacity-50"><CheckCircle2 className="w-3.5 h-3.5"/>Public Publish Live</button>
        <button onClick={async ()=>{
          setSaving(true);
          try {
        ...
        }} className="px-4 py-2 rounded-xl bg-indigo-600 text-white text-[11px] font-black flex items-center gap-1.5"><Plus className="w-3.5 h-3.5"/>ADD PRODUCT</button>
      </div>
    </div>
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-2.5 shadow-sm flex flex-col xl:flex-row gap-2.5">
      ...
      <label className="flex items-center gap-2 text-[10px] font-black text-slate-500">SELLER<SearchableSelect value={sellerFilter} onChange={e=>setSellerFilter(e.target.value)} className="min-w-[170px] px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700"><option value="all">All Sellers ({sellers.length})</option>{sellers.map((s:any)=><option key={s.id} value={s.id}>{s.nameEn}</option>)}</SearchableSelect></label>
      <label className="flex items-center gap-2 text-[10px] font-black text-slate-500">STATUS<SearchableSelect value={statusFilter} onChange={e=>setStatusFilter(e.target.value)} className="min-w-[130px] px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700"><option value="all">All ({products.length})</option><option value="published">Published</option><option value="hidden">Hidden / Draft</option></SearchableSelect></label>
      <label className="flex items-center gap-2 text-[10px] font-black text-slate-500">CATEGORY<SearchableSelect value={categoryFilter} onChange={e=>setCategoryFilter(e.target.value)} className="min-w-[180px] px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700"><option value="all">All Categories</option>{categories.map((c:any)=><option key={c.id} value={c.id}>{c.nameEn || c.name}</option>)}</SearchableSelect></label>
    </div>
