...
import { Eye, Image as ImageIcon, ShoppingBag, Star } from 'lucide-react';
...
export const ReadOnlyStorefrontPreview: React.FC<Props> = ({ order, visibility, device }) => {
  ...
  const renderSection = (id: string) => {
    ...
    if (id === 'homeCategories') return <section key={id} className="px-4 sm:px-6 py-7"><div className="mb-5"><div className="text-[10px] uppercase tracking-[0.2em] font-bold text-[#8F7137]">{isAr ? (home.categoriesSubtitleArabic || home.categoriesSubtitle || 'تصفح الأقسام') : (home.categoriesSubtitle || 'Browse Departments')}</div><h2 className="text-2xl font-serif font-bold mt-1">{isAr ? (home.categoriesTitleArabic || 'تسوق حسب الفئات') : (home.categoriesTitle || 'Explore by Category')}</h2></div>{publishedCategories.length ? <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{publishedCategories.map((category: any) => <div key={category.id} className="rounded-xl overflow-hidden border border-slate-200 bg-white"><div className="aspect-square bg-slate-100 flex items-center justify-center">{category.bannerUrl ? <img src={category.bannerUrl} alt={isAr ? category.nameAr : category.nameEn} className="w-full h-full object-contain" /> : <ImageIcon className="w-8 h-8 text-slate-300" />}</div><div className="p-3 text-xs font-bold">{isAr ? (category.nameAr || category.nameEn) : (category.nameEn || category.nameAr)}</div></div>)}</div> : renderEmptyData(isAr ? 'لا توجد فئات منشورة' : 'No published categories')}</section>;
    if (id === 'homeFeatured') return <section key={id} className="px-4 sm:px-6 py-7"><h2 className="text-xl font-black mb-4">{labels[id]}</h2>{renderProductCards(liveProducts.filter((p: any) => p.isFeatured))}</section>;
    ...
    if (id === 'homeNewArrivals') return <section key={id} className="px-4 sm:px-6 py-7"><h2 className="text-xl font-black mb-4">{labels[id]}</h2>{renderProductCards(liveProducts.filter((p: any) => p.isNewArrival))}</section>;
    if (id === 'homeBundles') return <section key={id} className="px-4 sm:px-6 py-7"><h2 className="text-xl font-black mb-4">{labels[id]}</h2>{renderEmptyData('No published bundles')}</section>;
    if (id === 'homeReviews') return <section key={id} className="px-4 sm:px-6 py-7"><h2 className="text-xl font-black mb-4 flex items-center gap-2"><Star className="w-5 h-5" /> {labels[id]}</h2>{renderEmptyData('No published customer reviews')}</section>;
    ...
    if (id === 'homeTrustBadges') return <section key={id} className="px-4 sm:px-6 py-7"><div className="grid grid-cols-2 gap-3">{['Verified', 'Delivery', 'Quality', 'Support'].map(item => <div key={item} className="rounded-xl border border-slate-200 bg-white p-4 text-center text-xs font-bold">{item}</div>)}</div></section>;
