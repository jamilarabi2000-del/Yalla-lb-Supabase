...
export const ReadOnlyStorefrontPreview: React.FC<Props> = ({ order, visibility, device }) => {
  ...
  const heroSlides = useMemo(() => {
    ...
    const base = media.length ? media.map((item: any, idx: number) => ({
      ...
      secondary: isAr ? (hero.secondaryBtnTextArabic || hero.secondaryBtnText) : hero.secondaryBtnText, promoCode: undefined,
  ...
  const cards = (items: any[]) => !items.length ? empty('No matching published products') : <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">{items.slice(0, 8).map((p: any) => { const name = isAr ? (p.arabicName || p.name) : p.name; const image = p.image || p.additionalImages?.[0]; const price = Number(p.priceUSD); const original = Number(p.originalPriceUSD); const discounted = Number.isFinite(price) && Number.isFinite(original) && original > price; return <article key={p.id} className="rounded-xl overflow-hidden border border-slate-200 bg-white"><div className="aspect-square bg-slate-100 flex items-center justify-center">{image ? <img src={image} alt={name || 'Product'} loading="lazy" className="w-full h-full object-contain"/> : <ImageIcon className="w-8 h-8 text-slate-300"/>}</div><div className="p-3"><div className="text-[11px] font-bold line-clamp-2 min-h-[2rem]">{name || 'Unnamed product'}</div><div className="mt-2 flex items-center gap-2"><span className="text-sm font-black">{Number.isFinite(price) ? `$${price.toFixed(2)}` : '—'}</span>{discounted ? <span className="text-[10px] text-slate-400 line-through">${original.toFixed(2)}</span> : null}</div>{p.stock <= 0 ? <div className="mt-1 text-[9px] font-bold text-rose-500">Out of stock</div> : null}</div></article>; })}</div>;
