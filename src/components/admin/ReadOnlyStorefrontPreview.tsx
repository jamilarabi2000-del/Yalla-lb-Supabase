...
export const ReadOnlyStorefrontPreview: React.FC<Props> = ({ order, visibility, device }) => {
  ...
  const renderHero = () => {
    ...
    return <section className="relative overflow-hidden bg-[#111] text-white rounded-[20px] h-[220px] sm:h-[300px] md:h-[360px] lg:h-[420px]">
      {currentHero.desktopUrl ? <><img src={currentHero.desktopUrl} alt="" className="absolute inset-0 w-full h-full object-cover blur-2xl opacity-40 scale-110"/><img src={currentHero.mobileUrl || currentHero.desktopUrl} alt={currentHero.title || ''} className={`absolute inset-0 w-full h-full ${fitClass}`} style={{ objectPosition: heroPosition || 'center', transform: heroZoom !== 100 ? `scale(${heroZoom / 100})` : undefined }} /></> : <div className="absolute inset-0 bg-slate-800"/>}
      ...
      <div className="relative z-10 h-full p-5 sm:p-8 flex flex-col justify-between"><div className="flex justify-between gap-3">{currentHero.badge ? <div className="flex items-center gap-2 text-xs font-bold"><Sparkles className="w-4 h-4 text-amber-200"/>{currentHero.badge}</div> : <span/>}{currentHero.promoCode ? <div className="text-[10px] font-mono px-2 py-1 rounded-full bg-black/50 border border-white/20 flex items-center gap-1"><Tag className="w-3 h-3"/>{currentHero.promoCode}</div> : null}</div><div><h2 className="text-2xl sm:text-4xl font-serif font-bold line-clamp-2">{currentHero.title || 'CMS Hero'}</h2>{currentHero.subtitle ? <p className="mt-2 text-xs sm:text-sm text-white/80 max-w-xl line-clamp-2">{currentHero.subtitle}</p> : null}</div><div className="flex items-center justify-between gap-3"><div>{currentHero.button ? <span className="inline-flex px-4 py-2 rounded-xl bg-[#B89753] text-white text-xs font-bold">{currentHero.button}</span> : null}{currentHero.secondary ? <span className="ml-2 inline-flex px-4 py-2 rounded-xl bg-white/20 border border-white/30 text-white text-xs font-bold">{currentHero.secondary}</span> : null}</div>{heroSlides.length > 1 ? <div className="flex items-center gap-2"><button type="button" onClick={() => setHeroIndex(i => (i - 1 + heroSlides.length) % heroSlides.length)} className="w-7 h-7 rounded-full bg-black/40 border border-white/20 flex items-center justify-center"><ChevronLeft className="w-4 h-4"/></button><span className="text-[10px]">{heroIndex + 1}/{heroSlides.length}</span><button type="button" onClick={() => setHeroIndex(i => (i + 1) % heroSlides.length)} className="w-7 h-7 rounded-full bg-black/40 border border-white/20 flex items-center justify-center"><ChevronRight className="w-4 h-4"/></button></div> : null}</div></div>
