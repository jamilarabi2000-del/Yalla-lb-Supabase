...
function NotificationsManager(){
  const empty={title_en:'',title_ar:'',body_en:'',body_ar:'',type:'announcement',status:'draft',target_audience:'all',target_page:'all',placement:'top',position_mode:'fixed',alignment:'center',font_family:'inherit',title_font_size:'18px',body_font_size:'14px',font_weight:'700',line_height:'1.4',letter_spacing:'0px',text_color:'#FFFFFF',title_color:'',background_color:'#171717',accent_color:'#B89753',button_background_color:'#B89753',button_text_color:'#171717',border_color:'transparent',border_width:'0px',border_radius:'16px',shadow:'0 12px 40px rgba(0,0,0,0.16)',opacity:1,max_width:'720px',padding:'14px 18px',icon_name:'✨',image_url:'',cta_text_en:'',cta_text_ar:'',cta_url:'',dismissible:true,show_once:false,auto_close_ms:'',start_at:'',end_at:'',sort_order:0};
  const [items,setItems]=useState<any[]>([]); const [form,setForm]=useState<any>(empty); const [editing,setEditing]=useState<string|null>(null); const [loading,setLoading]=useState(true); const [saving,setSaving]=useState(false); const [allowed,setAllowed]=useState(false);
  const {showToast=()=>{},language='en'}=useShop() as any;
  const refresh=async()=>{setLoading(true);const {data,error}=await supabase.from('notification_campaigns').select('*').order('sort_order',{ascending:true}).order('created_at',{ascending:false});if(error)showToast(error.message,'error');else setItems(data||[]);setLoading(false);};
  ...
  const setField=(k:string,v:any)=>setForm((x:any)=>({...x,[k]:v})); const reset=()=>{setEditing(null);setForm({...empty});};
  const edit=(item:any)=>{setEditing(item.id);setForm({...empty,...item,auto_close_ms:item.auto_close_ms??''});};
  const save=async()=>{if(!form.title_en.trim()||!form.body_en.trim()){showToast('English title and message are required.','warning');return;}setSaving(true);try{const {data:u}=await supabase.auth.getUser();const payload={...form,opacity:Number(form.opacity),auto_close_ms:form.auto_close_ms===''?null:Number(form.auto_close_ms),sort_order:Number(form.sort_order)||0,start_at:form.start_at||null,end_at:form.end_at||null,image_url:form.image_url||null,icon_name:form.icon_name||null,title_ar:form.title_ar||null,body_ar:form.body_ar||null,title_color:form.title_color||null,cta_text_en:form.cta_text_en||null,cta_text_ar:form.cta_text_ar||null,cta_url:form.cta_url||null};const q=editing?supabase.from('notification_campaigns').update(payload).eq('id',editing).select().single():supabase.from('notification_campaigns').insert({...payload,created_by:u.user?.id}).select().single();const {data,error}=await q;if(error)throw error;showToast(editing?'Notification updated.':'Notification created.','success');setEditing(data?.id||null);await refresh();}catch(e:any){showToast(e?.message||'Unable to save notification.','error');}finally{setSaving(false);}};
  const remove=async(id:string)=>{if(!window.confirm('Delete this notification campaign?'))return;const {error}=await supabase.from('notification_campaigns').delete().eq('id',id);if(error)showToast(error.message,'error');else{showToast('Notification deleted.','success');if(editing===id)reset();await refresh();}};
  const duplicate=async(item:any)=>{const {data:u}=await supabase.auth.getUser();const {id,created_at,updated_at,created_by,...copy}=item;const {error}=await supabase.from('notification_campaigns').insert({...copy,title_en:`${copy.title_en} (Copy)`,status:'draft',created_by:u.user?.id});if(error)showToast(error.message,'error');else{showToast('Draft copy created.','success');await refresh();}};
  const Preview=()=> <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400 mb-3">Live Preview</p><div dir={language==='ar'?'rtl':'ltr'} className="mx-auto flex items-center gap-3" style={{maxWidth:form.max_width,padding:form.padding,color:form.text_color,background:form.background_color,border:`${form.border_width} solid ${form.border_color}`,borderRadius:form.border_radius,boxShadow:form.shadow,textAlign:form.alignment,fontFamily:form.font_family==='inherit'?'inherit':form.font_family,lineHeight:form.line_height,letterSpacing:form.letter_spacing,opacity:Number(form.opacity)}}>{form.image_url&&<img src={form.image_url} alt="" className="w-10 h-10 rounded-xl object-cover shrink-0"/>}{!form.image_url&&form.icon_name&&<span className="text-xl shrink-0">{form.icon_name}</span>}<div className="min-w-0 flex-1"><div style={{color:form.title_color||form.text_color,fontSize:form.title_font_size,fontWeight:form.font_weight}}>{language==='ar'?(form.title_ar||form.title_en):form.title_en||'Notification title'}</div><div className="mt-1 opacity-90" style={{fontSize:form.body_font_size}}>{language==='ar'?(form.body_ar||form.body_en):form.body_en||'Notification message'}</div>{(form.cta_text_en||form.cta_text_ar)&&<span className="inline-flex mt-3 px-3.5 py-2 rounded-xl font-bold" style={{background:form.button_background_color,color:form.button_text_color}}>{language==='ar'?(form.cta_text_ar||form.cta_text_en):form.cta_text_en||form.cta_text_ar}</span>}</div>{form.dismissible&&<X className="w-4 h-4 shrink-0"/>}</div></div>;
  ...
  return <section className="space-y-5"><div className="bg-white border rounded-2xl p-5 flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-2xl font-black">Notifications</h2><p className="text-sm text-slate-500 mt-1">Customer-facing banners, alerts and promotional messages with full content, design, position, typography, audience and scheduling controls.</p></div><button onClick={reset} className="px-4 py-2.5 rounded-xl bg-slate-900 text-white font-bold flex items-center gap-2"><Plus className="w-4 h-4"/> New Notification</button></div>
  <div className="grid xl:grid-cols-[minmax(0,1fr)_420px] gap-5"><div className="bg-white border rounded-2xl p-5 space-y-5"><div className="flex items-center justify-between"><h3 className="font-black text-lg">{editing?'Edit Notification':'Create Notification'}</h3>{editing&&<button onClick={reset} className="text-xs font-bold text-slate-500">Cancel edit</button>}</div>
  <div className="grid md:grid-cols-2 gap-3"><TextField label="Title — English" k="title_en"/><TextField label="Title — Arabic" k="title_ar"/><label className="md:col-span-2 space-y-1 text-xs font-semibold text-slate-700"><span>Message — English</span><textarea value={form.body_en} onChange={e=>setField('body_en',e.target.value)} rows={3} className="w-full px-3 py-2.5 rounded-xl border border-slate-200"/></label><label className="md:col-span-2 space-y-1 text-xs font-semibold text-slate-700"><span>Message — Arabic</span><textarea value={form.body_ar} onChange={e=>setField('body_ar',e.target.value)} rows={3} className="w-full px-3 py-2.5 rounded-xl border border-slate-200"/></label><SelectField label="Type" k="type" options={['announcement','promotion','system','warning','success','info']}/><SelectField label="Status" k="status" options={['draft','published','scheduled','archived']}/><SelectField label="Audience" k="target_audience" options={['all','customers','sellers','logged_in','logged_out']}/><SelectField label="Page" k="target_page" options={['all','home','products','product_detail','checkout','account']}/><SelectField label="Position" k="placement" options={['top','top_left','top_center','top_right','center','bottom','bottom_left','bottom_center','bottom_right']}/><SelectField label="Position Mode" k="position_mode" options={['fixed','sticky','inline','overlay']}/><SelectField label="Alignment" k="alignment" options={['left','center','right']}/><TextField label="Font Family" k="font_family" placeholder="inherit, Inter, Tajawal..."/><TextField label="Icon / Emoji" k="icon_name" placeholder="✨"/><TextField label="Image URL" k="image_url"/><TextField label="CTA URL" k="cta_url" placeholder="/products"/><TextField label="CTA Text — English" k="cta_text_en"/><TextField label="CTA Text — Arabic" k="cta_text_ar"/></div>
  <div className="border-t pt-5"><h4 className="font-black mb-3">Typography</h4><div className="grid md:grid-cols-3 gap-3"><TextField label="Title Size" k="title_font_size" placeholder="18px"/><TextField label="Body Size" k="body_font_size" placeholder="14px"/><TextField label="Weight" k="font_weight" placeholder="700"/><TextField label="Line Height" k="line_height" placeholder="1.4"/><TextField label="Letter Spacing" k="letter_spacing" placeholder="0px"/><TextField label="Max Width" k="max_width" placeholder="720px"/></div></div>
  <div className="border-t pt-5"><h4 className="font-black mb-3">Colors & Shape</h4><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{[['Text Color','text_color'],['Title Color','title_color'],['Background','background_color'],['Accent','accent_color'],['Button Background','button_background_color'],['Button Text','button_text_color'],['Border','border_color']].map(([label,k])=><label key={k} className="space-y-1 text-xs font-semibold text-slate-700"><span>{label}</span><div className="flex gap-2"><input type="color" value={/^#[0-9A-Fa-f]{6}$/.test(form[k]||'')?form[k]:'#000000'} onChange={e=>setField(k,e.target.value)} className="w-11 h-10 rounded-lg border p-1"/><input value={form[k]??''} onChange={e=>setField(k,e.target.value)} className="flex-1 px-3 py-2 rounded-xl border border-slate-200"/></div></label>)}<TextField label="Border Width" k="border_width" placeholder="0px"/><TextField label="Radius" k="border_radius" placeholder="16px"/><TextField label="Shadow" k="shadow" placeholder="0 12px 40px rgba(...)"/><TextField label="Padding" k="padding" placeholder="14px 18px"/><TextField label="Opacity" k="opacity" placeholder="1"/></div></div>
  <div className="border-t pt-5"><h4 className="font-black mb-3">Display Rules</h4><div className="grid md:grid-cols-2 gap-3"><TextField label="Start At" k="start_at" placeholder="2026-09-20T12:00:00Z"/><TextField label="End At" k="end_at" placeholder="2026-09-30T23:59:59Z"/><TextField label="Auto Close (ms)" k="auto_close_ms" placeholder="0 = never"/><TextField label="Sort Order" k="sort_order" placeholder="0"/><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={!!form.dismissible} onChange={e=>setField('dismissible',e.target.checked)}/> Allow customer to dismiss</label><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={!!form.show_once} onChange={e=>setField('show_once',e.target.checked)}/> Show once per browser</label></div></div>
  <div className="flex flex-wrap gap-2 pt-2"><button disabled={saving} onClick={save} className="px-5 py-2.5 rounded-xl bg-slate-900 text-white font-black disabled:opacity-50"><Save className="w-4 h-4 inline mr-2"/>{saving?'Saving...':editing?'Update Notification':'Save Notification'}</button><button onClick={reset} className="px-5 py-2.5 rounded-xl border font-bold">Reset</button></div></div>
  <div className="space-y-5"><Preview/><div className="bg-white border rounded-2xl p-5"><h3 className="font-black mb-3">Saved Notifications</h3>{loading?<p className="text-sm text-slate-500">Loading...</p>:!items.length?<p className="text-sm text-slate-500">No notification campaigns yet.</p>:<div className="space-y-2 max-h-[650px] overflow-y-auto">{items.map(item=><div key={item.id} className="border rounded-xl p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="font-black truncate">{item.title_en}</div><div className="text-xs text-slate-500 mt-1">{item.status} · {item.target_audience} · {item.target_page} · {item.placement}</div></div><div className="flex gap-1"><button onClick={()=>edit(item)} className="px-2 py-1 rounded-lg bg-slate-100 text-xs font-bold">Edit</button><button onClick={()=>duplicate(item)} className="px-2 py-1 rounded-lg bg-slate-100 text-xs font-bold">Copy</button><button onClick={()=>remove(item.id)} className="p-1 rounded-lg text-rose-600 hover:bg-rose-50"><Trash2 className="w-4 h-4"/></button></div></div></div>)}</div>}</div></div></div></section>;
}
