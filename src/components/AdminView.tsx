...
const emptyProduct = () => ({
  ...
  regularPriceUSD: 15, promoPriceUSD: '', discountPercentage: '', stock: 25, lowStockThreshold: 5, lowStockNotice: '', customStockLabel: '', costPriceUSD: '',
...
function ProductsManager() {
  ...
  const validate = (f:any, id?:string) => {
    const stock = Number(f.stock), price = Number(f.regularPriceUSD);
  ...
  const payload = (f:any, published:boolean): Omit<Product,'id'> => ({
    ...
    origin: String(f.origin||'Lebanon').trim() || 'Lebanon', regularPriceUSD:Number(f.regularPriceUSD), promoPriceUSD:Number(f.promoPriceUSD)>0?Number(f.promoPriceUSD):undefined, discountPercentage:String(f.discountPercentage)===''?undefined:Number(f.discountPercentage), rating:0, reviewsCount:0,
