...
export const ProductDetailView: React.FC = () => {
  ...
  const hasPromo = Number(p.originalPriceUSD || 0) > 0 && Number(p.originalPriceUSD) < Number(p.priceUSD);
  const displayPriceUSD = hasPromo ? Number(p.originalPriceUSD) : Number(p.priceUSD);
  const displayOriginalPriceUSD = hasPromo ? Number(p.priceUSD) : null;
  const price = shop.formatPrice?.(displayPriceUSD) || '
...
export default ProductDetailView;
 + displayPriceUSD.toFixed(2);
  const originalPrice = displayOriginalPriceUSD ? (shop.formatPrice?.(displayOriginalPriceUSD) || '
  const inStock = Number(p.stock || 0) > 0;
  const maxQty = Math.max(1, Number(p.stock || 1));
  const discount = p.discountPercentage || (p.originalPriceUSD && p.priceUSD < p.originalPriceUSD ? Math.round((1 - p.priceUSD / p.originalPriceUSD) * 100) : 0);
...
export default ProductDetailView;
 + displayOriginalPriceUSD.toFixed(2)) : null;
  const inStock = Number(p.stock || 0) > 0;
  const maxQty = Math.max(1, Number(p.stock || 1));
  const discount = p.discountPercentage || (p.originalPriceUSD && p.priceUSD < p.originalPriceUSD ? Math.round((1 - p.priceUSD / p.originalPriceUSD) * 100) : 0);
