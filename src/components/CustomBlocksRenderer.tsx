...
export const CustomBlocksRenderer: React.FC<CustomBlocksRendererProps> = ({ page, position, onEditBlock }) => {
  ...
  const handleAction = (url?: string) => {
    ...
    if (url === '/products' || url === 'products') {
      setSelectedCategory('all'); setActiveTab('products');
    } else if (url === '/checkout' || url === 'checkout') {
      setActiveTab('checkout');
    } else if (url.startsWith('category:')) {
      setSelectedCategory(url.replace('category:', '')); setActiveTab('products');
  ...
  return (
    <div className="w-full space-y-6 max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
      {blocks.map(block => {
        ...
        if (block.bgStyle === 'gold_gradient') bgClass = 'bg-gradient-to-r from-amber-950 via-[#1f190e] to-slate-950 text-white border border-amber-500/30';
        else if (block.bgStyle === 'emerald_gradient') bgClass = 'bg-gradient-to-r from-emerald-950 via-teal-950 to-slate-950 text-white border border-emerald-500/30';
        else if (block.bgStyle === 'light') bgClass = 'bg-white text-slate-900 border border-slate-200 shadow-sm';
        else if (block.bgStyle === 'glass') bgClass = 'bg-slate-900/80 backdrop-blur-md text-white border border-white/10 shadow-lg';
        else if (block.bgStyle === 'custom_image' && block.imageUrl) bgClass = 'relative bg-slate-900 text-white overflow-hidden border border-slate-700';
