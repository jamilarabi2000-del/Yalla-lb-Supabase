      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 text-center relative z-10 flex flex-col items-center">
        ...
        {(visibility.footerContact || isVisualEditMode) && (footerData.phone || footerData.email || footerData.address || footerData.hours) && (
          <div className={`w-full mb-6 py-4 px-6 rounded-xl bg-white/[0.03] border border-white/10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-center relative ${!visibility.footerContact && isVisualEditMode ? 'opacity-70 border-2 border-dashed border-rose-500/80' : ''}`}>
            ...
            {showPhoneSupport && footerData.phone && <div className="flex flex-col items-center"><span className="text-[10px] font-bold uppercase tracking-wider text-[#B89753] mb-1">{language === 'ar' ? 'الهاتف' : 'Phone'}</span><a href={`tel:${footerData.phone}`} className="inline-flex items-center min-h-6 text-xs text-neutral-300 hover:text-white transition-colors">{footerData.phone}</a></div>}
            {footerData.email && <div className="flex flex-col items-center"><span className="text-[10px] font-bold uppercase tracking-wider text-[#B89753] mb-1">{language === 'ar' ? 'البريد الإلكتروني' : 'Email'}</span><a href={`mailto:${footerData.email}`} className="inline-flex items-center min-h-6 text-xs text-neutral-300 hover:text-white transition-colors">{footerData.email}</a></div>}
