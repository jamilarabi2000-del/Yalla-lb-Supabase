      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 text-center relative z-10 flex flex-col items-center">
        ...
        {(visibility.footerSocial || isVisualEditMode) && (
          <div className={`flex items-center justify-center gap-3 mb-6 relative ${!visibility.footerSocial && isVisualEditMode ? 'opacity-70 border-2 border-dashed border-rose-500/80 rounded-2xl p-2' : ''}`}>
            {(siteContent?.socialLinks?.instagram || isVisualEditMode) && <a href={safeHref(siteContent?.socialLinks?.instagram, 'https://instagram.com/yalla.lb')} target="_blank" rel="noopener noreferrer" aria-label="Instagram" data-brand="instagram" className="w-9 h-9 rounded-lg bg-white/[0.04] border border-white/10 text-neutral-300 flex items-center justify-center shadow-2xs cursor-pointer social-link"><BrandIcon brand="instagram" className="w-4 h-4" /></a>}
