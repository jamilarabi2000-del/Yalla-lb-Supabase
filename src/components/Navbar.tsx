  ...
  return (
    <header data-cms-element="navbar" className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-[#E5E5E5] shadow-2xs">
      ...
      <div className="max-w-screen-2xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 lg:h-18 gap-2 sm:gap-4">
          ...
          {showSearch && (
            <form onSubmit={handleSearchSubmit} className="hidden md:flex flex-1 max-w-md items-center relative group">
              ...
              {searchQuery && <button type="button" onClick={() => setSearchQuery('')} className={`absolute ${language === 'ar' ? 'left-2.5' : 'right-2.5'} px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-[#737373] hover:text-[#171717] bg-slate-200/70 hover:bg-slate-200 rounded-md transition-colors cursor-pointer`}>{t('clear')}</button>}
            </form>
          )}

          <nav className="hidden lg:flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
            <button id="nav-home-btn" onClick={() => setActiveTab('home')} className={`px-3 py-2 rounded-lg transition-all cursor-pointer ${activeTab === 'home' ? 'text-[#171717] bg-[#F8F8F6] border border-[#E5E5E5]' : 'text-[#737373] hover:text-[#171717] hover:bg-[#F8F8F6]'}`}>{t('home')}</button>
            <div className="relative">
              <button id="nav-categories-dropdown-btn" onClick={() => setCategoryDropdownOpen(!categoryDropdownOpen)} className="px-3 py-2 rounded-lg transition-all cursor-pointer text-[#737373] hover:text-[#171717] hover:bg-[#F8F8F6] flex items-center gap-1.5"><span>{t('categories')}</span><ChevronDown className={`w-3.5 h-3.5 text-[#8F7137] transition-transform duration-200 ${categoryDropdownOpen ? 'rotate-180' : ''}`} /></button>
              {categoryDropdownOpen && <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-xl shadow-lg border border-[#E5E5E5] py-2 z-50 max-h-[70vh] overflow-y-auto animate-in fade-in slide-in-from-top-2"><div className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-[#737373] border-b border-[#E5E5E5] sticky top-0 bg-white z-10">{t('categories')}</div>{categoriesList.map(cat => <button key={cat.id} onClick={() => { setSelectedCategory(cat.id); setActiveTab('products'); setCategoryDropdownOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="w-full text-left px-4 py-2 text-xs font-semibold text-[#171717] hover:bg-[#F8F8F6] hover:text-[#8F7137] flex items-center gap-2.5 transition-colors cursor-pointer"><span className="text-sm">{cat.icon}</span><span>{cat.name}</span></button>)}</div>}
            </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            {showLanguage && <button id="global-language-switcher-btn" onClick={() => setLanguage(language === 'en' ? 'ar' : 'en')} className="flex items-center gap-1 sm:gap-1.5 px-2.5 py-1.5 rounded-lg bg-[#F8F8F6] hover:bg-slate-200 text-[#171717] border border-[#E5E5E5] font-bold text-xs transition-colors cursor-pointer flex-shrink-0" title="Switch System Language / تغيير لغة النظام"><Globe2 className="w-3.5 h-3.5 text-[#8F7137] flex-shrink-0" /><span className="font-semibold text-xs leading-none">{language === 'en' ? 'العربية' : 'English'}</span></button>}

            {showSearch && <button id="mobile-search-toggle-btn" onClick={() => setSearchOpen(!searchOpen)} className="md:hidden p-2 rounded-lg text-[#737373] hover:text-[#171717] hover:bg-[#F8F8F6] transition-colors flex items-center justify-center flex-shrink-0 cursor-pointer border border-[#E5E5E5]" aria-label="Search"><Search className="w-4 h-4 text-[#8F7137]" /></button>}
        ...
        {showSearch && searchOpen && <div className="md:hidden pb-3 pt-1 px-2"><form onSubmit={handleSearchSubmit} className="relative"><Search className={`absolute ${language === 'ar' ? 'right-3.5 top-3' : 'left-3.5 top-3'} w-4 h-4 text-[#8F7137]`} /><input id="mobile-search-input" type="text" placeholder={language === 'ar' ? (siteContent?.navbar?.searchPlaceholderArabic || siteContent?.navbar?.searchPlaceholder || t('searchPlaceholder')) : (siteContent?.navbar?.searchPlaceholder || t('searchPlaceholder'))} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full ${language === 'ar' ? 'pr-10 pl-16 text-right' : 'pl-10 pr-16 text-left'} py-2.5 text-xs bg-[#F8F8F6] text-[#171717] rounded-xl border border-[#E5E5E5] focus:border-[#8F7137] focus:bg-white focus:ring-2 focus:ring-[#8F7137]/20 focus:outline-none shadow-2xs`} />{searchQuery && <button type="button" onClick={() => setSearchQuery('')} className={`absolute ${language === 'ar' ? 'left-2.5' : 'right-2.5'} top-2 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-[#737373] bg-slate-200 rounded-md cursor-pointer`}>{t('clear')}</button>}</form></div>}

        {mobileMenuOpen && <div className="lg:hidden py-4 border-t border-[#E5E5E5] bg-white rounded-b-xl px-3 space-y-2 shadow-lg">
          <button onClick={() => { setActiveTab('favorites'); setMobileMenuOpen(false); }} className={`w-full text-left px-4 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center justify-between transition-colors ${activeTab === 'favorites' ? 'bg-rose-50 text-rose-700 border border-rose-200' : 'text-[#737373] hover:bg-[#F8F8F6]'}`}><div className="flex items-center gap-2.5"><Heart className={`w-4 h-4 text-[#C62828] ${activeTab === 'favorites' ? 'fill-rose-500' : ''}`} /><span>{t('wishlist')}</span></div>{wishlist.length > 0 && <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-100 text-rose-700">{wishlist.length}</span>}</button>

          <button onClick={() => { setActiveTab('account'); setMobileMenuOpen(false); }} className={`w-full text-left px-4 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2.5 ${activeTab === 'account' ? 'bg-amber-50 text-[#171717] border border-amber-200' : 'text-[#737373] hover:bg-[#F8F8F6]'}`}><User className="w-4 h-4 text-[#8F7137]" /><span>{t('myAccountAndOrders')}</span></button>

          <div className="py-2 px-3 text-[10px] font-bold uppercase tracking-widest text-[#737373] border-t border-[#E5E5E5] mt-2">{t('categories')}</div>
          <div className="grid grid-cols-2 gap-1.5 pb-2">{categoriesList.map(cat => <button key={`mobile-cat-${cat.id}`} onClick={() => { setSelectedCategory(cat.id); setActiveTab('products'); setMobileMenuOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="text-left px-3 py-2 rounded-lg text-xs font-medium text-[#171717] hover:bg-[#F8F8F6] hover:text-[#8F7137] flex items-center gap-2 transition-colors cursor-pointer bg-[#F8F8F6] border border-[#E5E5E5]"><span>{cat.icon}</span><span className="truncate">{cat.name}</span></button>)}</div>
