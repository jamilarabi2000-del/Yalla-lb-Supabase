...
export const dbMonitor={getLogs:()=>[...logs],clearLogs:()=>{logs.length=0},subscribe:(_fn:(record:FirestoreLogRecord,all:FirestoreLogRecord[])=>void)=>()=>{},getSummary:():SyncDiagnosticsSummary=>({totalOperations:logs.length,totalReads:0,totalWrites:0,successfulWrites:0,failedWrites:0,writeSuccessRate:100,avgLatencyMs:0,lastSuccessfulSync:null,lastFailedSync:null,lastError:null,activeMonitoredPaths:[]})};
...
export const redactPII=(data:any)=>data;
