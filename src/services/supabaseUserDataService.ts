...
export const supabaseUserDataService = {
  ...
  async fetchCart(userId: string): Promise<CartItem[] | null> {
    try { const { data, error } = await supabase.from('carts').select('items').eq('user_id', userId).maybeSingle(); if (error) { console.warn('[supabaseUserDataService] fetchCart error:', error.message); return null; } return data && Array.isArray(data.items) ? data.items as CartItem[] : null; } catch (err) { console.warn('[supabaseUserDataService] fetchCart error:', errorMessage(err)); return null; }
  },

  async saveCart(userId: string, items: CartItem[]): Promise<void> {
    try { const { error } = await supabase.from('carts').upsert({ user_id: userId, items, updated_at: new Date().toISOString() }, { onConflict: 'user_id' }); if (error) console.warn('[supabaseUserDataService] saveCart error:', error.message); } catch (err) { console.warn('[supabaseUserDataService] saveCart error:', errorMessage(err)); }
  },

  async fetchWishlist(userId: string): Promise<string[] | null> {
    try { const { data, error } = await supabase.from('wishlists').select('product_ids').eq('user_id', userId).maybeSingle(); if (error) { console.warn('[supabaseUserDataService] fetchWishlist error:', error.message); return null; } return data && Array.isArray(data.product_ids) ? data.product_ids as string[] : null; } catch (err) { console.warn('[supabaseUserDataService] fetchWishlist error:', errorMessage(err)); return null; }
  },

  async saveWishlist(userId: string, productIds: string[]): Promise<void> {
    try { const { error } = await supabase.from('wishlists').upsert({ user_id: userId, product_ids: productIds, updated_at: new Date().toISOString() }, { onConflict: 'user_id' }); if (error) console.warn('[supabaseUserDataService] saveWishlist error:', error.message); } catch (err) { console.warn('[supabaseUserDataService] saveWishlist error:', errorMessage(err)); }
  },
  ...
  async addReview(review: Omit<Review, 'id' | 'createdAt'> & { id?: string }): Promise<void> {
    ...
    const { error } = await supabase.from('reviews').insert(payload); if (error) { console.error('[supabaseUserDataService] addReview error:', error); throw error; }
  },
};