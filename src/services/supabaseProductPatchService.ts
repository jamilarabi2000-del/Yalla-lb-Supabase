...
function buildPayload(updates: ProductUpdates){
  ...
  // The legacy price_usd and original_price_usd columns are generated by the
  // database and must never be included in UPDATE payloads.
  if(hasOwn(updates,'priceUSD') && updates.priceUSD !== undefined){
    payload.regular_price = updates.priceUSD;
  }
  if(hasOwn(updates,'originalPriceUSD') && updates.originalPriceUSD !== undefined){
    payload.promo_price = updates.originalPriceUSD;
  }
